...
1. **Allergen disclosures.** Any response that suggests a specific recipe or ingredient must include a visible allergen notice directing the user to verify ingredient safety themselves. This applies whether or not the user mentioned allergies. Please confirm with your build partner that this is part of the response format from day one. Retrofitting disclaimers into a conversational product is messy and we've seen companies get in trouble for inconsistency. To be clear: I don't want this to be something the model includes only when it remembers to. It needs to appear reliably on every recipe response, every time, worded consistently, no matter how the model phrases the rest of its answer. How your build partner guarantees the user actually sees it, rather than trusting the model to comply, is exactly the kind of thing I'll want walked through.
