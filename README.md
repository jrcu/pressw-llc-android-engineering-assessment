...
- All LLM calls routed through the **Vercel AI SDK** (no model-specific SDKs directly)
- A **native Android chat frontend**, built with Kotlin and Jetpack Compose
- Docker Compose for local development, so we can run your backend with a single command. If your scope needs a database, use a local Postgres instance in your Docker Compose setup (no hosted database required).
- Include standard Android Studio run instructions for the frontend (Docker not required there)

We'll run and exercise your system locally (Docker Compose for the backend, Android Studio for the frontend), so make sure that path is clean. A hosted or live deployment is not required for v1, so spend that time on the product instead.
...
- **Use AI coding tools.** We do, and we expect you to (Claude Code, Cursor, Copilot, whatever you reach for). We're evaluating your judgment and your output, not whether you typed it by hand.
